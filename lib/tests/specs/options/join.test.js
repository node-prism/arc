...
export default testSuite(async ({ describe }) => {
    describe("join", ({ test }) => {
        test("works", () => {
            ...
            expect(res.userTickets).toEqual(tks);
        });
        test("will overwrite original property", () => {
            ...
            expect(res.tickets).toEqual(tks);
        });
        test("creates the 'as' property even when nothing matches", () => {
            ...
            expect(res).toHaveProperty("userTickets");
            expect(res.userTickets).toEqual([]);
        });
        ...
        test("nested joins", () => {
            ...
            expect(res).toHaveProperty("userTickets");
            res["userTickets"].forEach((t) => {
                expect(t).toHaveProperty("userSeats");
            });
            expect(res.userTickets[0]).toEqual({ ...tks[0], userSeats: [sts[0]] });
            expect(res.userTickets[1]).toEqual({ ...tks[1], userSeats: [sts[1]] });
        });
        test("with from dot notation, accessing array of objects", () => {
            ...
            expect(res.userItems).toEqual(its);
        });
    });
});
