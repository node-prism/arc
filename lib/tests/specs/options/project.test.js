...
export default testSuite(async ({ describe }) => {
    describe("project", ({ test }) => {
        ...
        test("explicit - ID_KEY implicitly included", () => {
            ...
            collection.insert({ a: 2, b: 2, c: 2 });
            collection.insert({ a: 3, b: 3, c: 3 });
            const foundWithId = collection.find({ a: 1 }, {
                project: {
                    b: 1,
                    c: 0,
                    _created_at: 0,
        ...
        describe("aggregation", ({ test }) => {
            test("$floor, $ceil, $sub, $add, $mult, $div", () => {
                ...
                collection.insert({ a: 2, b: 2, c: 2 });
                collection.insert({ a: 3, b: 3, c: 3 });
                const found = collection.find({ a: 1 }, {
                    aggregate: {
                        flooredC: { $floor: "c" },
                        ceiledC: { $ceil: "c" },
                        subbed1: { $sub: ["c", "a"] },
            ...
            test("more realistic use-case", () => {
                ...
                const found = nrml(collection.find({ $has: ["math", "english", "science"] }, {
                    aggregate: {
            ...
            test("accessing properties with dot notation", () => {
                ...
                const found = collection.find({ a: { b: { c: 1 } } }, {
                    aggregate: {
                        d: { $add: ["a.b.c", 1] },
                    },
                    project: {
            ...
            test("$fn", () => {
                ...
                const found = nrml(collection.find({ $has: ["first", "last"] }, {
                    aggregate: {
